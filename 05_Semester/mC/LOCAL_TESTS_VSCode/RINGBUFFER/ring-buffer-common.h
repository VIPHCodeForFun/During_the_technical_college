 ...
 * @param[in] valueIncr  the value increment (n-th value = \n firstValue + (n-1) * \p valueIncr)
 * @return Number of successful pushes
 */

uint16_t pushMultiple(RingBuffer *ringBuffer, const uint16_t nValue, const uint8_t firstValue, const uint8_t valueIncr)
{
 ...
 * @return Number of matching values according to \p expectedFirstValue and \p expectedValueIncr
 */
uint16_t popMultiple(RingBuffer *ringBuffer, const uint16_t nValue, const uint8_t expectedFirstValue, const uint8_t expectedValueIncr)
{
    const uint16_t initialSize = ringBufferSize(ringBuffer);
    ...
    for (uint16_t i = 0; i < nValue; ++i)
    {
        ...
        if (!success || expectedSize != ringBufferSize(ringBuffer) || value != expectedValue)
        {
            break;
        }
