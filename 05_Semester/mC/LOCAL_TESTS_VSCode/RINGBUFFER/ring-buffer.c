...
void testRingBufferPushPopMultiple()
{
	...
	const uint16_t nValuePushed = pushMultiple(&ringBuffer, nValueExpected, 1, 50);
...
void testRingBufferPushBeyond()
{
	...
	// Trying to push the buffer beyond its capacity, the number of successful pushes must match the buffer capacity
	const uint8_t firstValue = 1;
	const uint16_t nValuePushed = pushMultiple(&ringBuffer, 2 * ringBufferCapacity(), firstValue, 30);
...
void testRingBufferPopEmpty()
{
	...
	}
	{
		...
		const uint16_t nValuePushed = pushMultiple(&ringBuffer, nValueExpected, 1, 1);
		assert(nValuePushed == nValueExpected);
...
void testRingBufferSize()
{
	...
	{
		// With an empty buffer, size must remain zero when attempting to remove a value
		uint8_t value = 255;
...
void testRingBufferFull()
{
	...
	{
		// Pushing values until the reported capacity has been reached. The buffer must be full now.
		const uint8_t nValuePushed = pushMultiple(&ringBuffer, ringBufferCapacity(), 0, 2);
		assert(nValuePushed == ringBufferCapacity());
		assert(ringBufferFull(&ringBuffer));
	}
	{
...
void testRingBufferPeek()
{
	...
	{
		// When we peek a value, it must remain in the buffer and the size must be unchanged
		// First, push a few values
		...
		// The value may be "peeked"  a few times, the value itself and the buffer size must remain unchanged.
...
{
	// This test checks the correct wrapping of the internal buffer read and write positions
